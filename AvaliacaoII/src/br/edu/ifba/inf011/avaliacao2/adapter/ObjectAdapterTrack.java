...
public class ObjectAdapterTrack implements Track {
    ...
    public void render(Integer init, Integer duration) {
        if (!isFullyLoaded) {
            ...
            System.out.println("[Adapter de Objeto] Carregando todos os bytes para a memória de " + this.streamName + ".");
        }
        Integer inicio = Math.min(init, this.getDurationInSeconds());
        Integer fim = Math.min(init + duration, this.getDurationInSeconds());
        System.out.println("[Adapter de Objeto] Renderizando os bytes de " + inicio  + " segundos até " + fim + " segundos.");
