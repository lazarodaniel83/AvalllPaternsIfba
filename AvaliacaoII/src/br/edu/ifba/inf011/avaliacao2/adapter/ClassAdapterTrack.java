...
public class ClassAdapterTrack extends HDDBinaryReader implements Track {
    ...
    public void render(Integer init, Integer duration) {
        if (this.rawBuffer == null) {
            ...
            System.out.println("[Adapter de Classe] Carregando todos os bytes para a memória de " + this.streamName + " utilizando o rawBuffer herdado (otimização de memória).");
        }
        Integer inicio = Math.min(init, this.getDurationInSeconds());
        Integer fim = Math.min(init + duration, this.getDurationInSeconds());
        System.out.println("[Adapter de Classe] Renderizando os bytes de " + inicio  + " segundos até " + fim + " segundos.");
    }
}
